...
let make_void_return () =
  Toyc_compiler__Ast.Return None
...
let assert_semantic_error ?(expect_not_all_paths_return=false) name prog expected_msg =
  ...
  | Toyc_compiler.NotAllPathsReturn msg ->
      print_result name ("NotAllPathsReturn: " ^ msg);
      assert_failure (name ^ " should raise Semantic_error, but got NotAllPathsReturn: " ^ msg)
  | e -> raise e
...
let test_if_else_return_ok _ =
  let body = Toyc_compiler__Ast.Block [
    Toyc_compiler__Ast.If (Toyc_compiler__Ast.Int 1, make_return 1, Some (make_return 2))
  ] in
  let prog = [make_main_func body] in
...
let test_while_ok _ =
  let body = Toyc_compiler__Ast.Block [
    Toyc_compiler__Ast.While (Toyc_compiler__Ast.Int 1, Toyc_compiler__Ast.Block [make_return 1])
  ] in
  let prog = [make_main_func body] in
  assert_semantic_ok "while_ok" prog

let test_func_call_ok _ =
  let foo = make_func "foo" ~params:[Toyc_compiler__Ast.Param "x"] (Toyc_compiler__Ast.Block [make_return 1]) in
  let main_body = Toyc_compiler__Ast.Block [
    Toyc_compiler__Ast.Expr (Toyc_compiler__Ast.Call ("foo", [Toyc_compiler__Ast.Int 10]));
    make_return 0
  ] in
...
let test_void_func_ok _ =
  let foo = make_func "foo" ~ret_type:Toyc_compiler__Ast.Void (Toyc_compiler__Ast.Block [make_void_return ()]) in
  let main_body = Toyc_compiler__Ast.Block [
    Toyc_compiler__Ast.Expr (Toyc_compiler__Ast.Call ("foo", []));
    make_return 0
  ] in
  let prog = [foo; make_main_func main_body] in
...
let test_complex_expr_ok _ =
  let body = Toyc_compiler__Ast.Block [
    ...
    Toyc_compiler__Ast.Expr (Toyc_compiler__Ast.BinaryOp (Toyc_compiler__Ast.And,
      Toyc_compiler__Ast.BinaryOp (Toyc_compiler__Ast.Lt, Toyc_compiler__Ast.Int 1, Toyc_compiler__Ast.Int 2),
      Toyc_compiler__Ast.BinaryOp (Toyc_compiler__Ast.Gt, Toyc_compiler__Ast.Int 3, Toyc_compiler__Ast.Int 0)));
    make_return 0
  ] in
  let prog = [make_main_func body] in
...
let test_main_with_param _ =
  let prog = [make_func "main" ~params:[Toyc_compiler__Ast.Param "a"] (Toyc_compiler__Ast.Block [make_return 0])] in
...
let test_main_no_return _ =
  ...
  assert_semantic_error ~expect_not_all_paths_return:true "main_no_return" prog "int 类型函数 main 不是所有路径都返回值"
...
let test_func_call_param_count _ =
  let foo = make_func "foo" ~params:[Toyc_compiler__Ast.Param "x"] ~ret_type:Toyc_compiler__Ast.Void (Toyc_compiler__Ast.Block [make_void_return ()]) in
  let main_body = Toyc_compiler__Ast.Block [
    Toyc_compiler__Ast.Expr (Toyc_compiler__Ast.Call ("foo", [Toyc_compiler__Ast.Int 1; Toyc_compiler__Ast.Int 2]));
    make_return 0
  ] in
  let prog = [foo; make_main_func main_body] in
...
let test_func_call_param_type _ =
  let foo = make_func "foo" ~params:[Toyc_compiler__Ast.Param "x"] ~ret_type:Toyc_compiler__Ast.Void (Toyc_compiler__Ast.Block [make_void_return ()]) in
  let bar = make_func "bar" ~ret_type:Toyc_compiler__Ast.Void (Toyc_compiler__Ast.Block [make_void_return ()]) in
  let main_body = Toyc_compiler__Ast.Block [
    Toyc_compiler__Ast.Expr (Toyc_compiler__Ast.Call ("foo", [Toyc_compiler__Ast.Call ("bar", [])]));
    make_return 0
  ] in
  let prog = [foo; bar; make_main_func main_body] in
...
let test_void_func_param_count _ =
  let foo = make_func "foo" ~params:[] ~ret_type:Toyc_compiler__Ast.Void (Toyc_compiler__Ast.Block [make_void_return ()]) in
  let main_body = Toyc_compiler__Ast.Block [
    Toyc_compiler__Ast.Expr (Toyc_compiler__Ast.Call ("foo", [Toyc_compiler__Ast.Int 1]));
    make_return 0
  ] in
  let prog = [foo; make_main_func main_body] in
...
let test_call_undeclared_func _ =
  let main_body = Toyc_compiler__Ast.Block [
    Toyc_compiler__Ast.Expr (Toyc_compiler__Ast.Call ("bar", [Toyc_compiler__Ast.Int 1]));
    make_return 0
  ] in
...
let test_assign_func_name _ =
  let foo = make_func "foo" (Toyc_compiler__Ast.Block [make_void_return ()]) in
  let body = Toyc_compiler__Ast.Block [
    Toyc_compiler__Ast.Assign ("foo", Toyc_compiler__Ast.Int 1);
    make_return 0
  ] in
...
let test_void_func_return_value _ =
  let foo = make_func "foo" ~ret_type:Toyc_compiler__Ast.Void (Toyc_compiler__Ast.Block [make_return 1]) in
...
let test_if_condition_type _ =
  let body = Toyc_compiler__Ast.Block [
    Toyc_compiler__Ast.If (Toyc_compiler__Ast.Call ("foo", []), make_return 1, Some (make_return 0))
  ] in
  let foo = make_func "foo" ~ret_type:Toyc_compiler__Ast.Void (Toyc_compiler__Ast.Block [make_void_return ()]) in
  let prog = [foo; make_main_func body] in
...
let test_break_continue_in_loop _ =
  let body = Toyc_compiler__Ast.Block [
    Toyc_compiler__Ast.While (Toyc_compiler__Ast.Int 1, Toyc_compiler__Ast.Block [
      Toyc_compiler__Ast.Break;
      Toyc_compiler__Ast.Continue;
      make_return 0
    ]);
    make_return 0
  ] in
...
let test_void_func_no_return_ok _ =
  let foo = make_func "foo" ~ret_type:Toyc_compiler__Ast.Void (Toyc_compiler__Ast.Block []) in
  let main_body = Toyc_compiler__Ast.Block [
    Toyc_compiler__Ast.Expr (Toyc_compiler__Ast.Call ("foo", []));
    make_return 0
  ] in
...
let test_func_name_duplicate _ =
  ...
  let prog = [foo1; foo2; make_main_func (Toyc_compiler__Ast.Block [make_return 0])] in
  assert_semantic_error "func_name_duplicate" prog "重复定义的函数名：foo"

let test_int_func_no_return _ =
  let foo = make_func "foo" ~ret_type:Toyc_compiler__Ast.Int (Toyc_compiler__Ast.Block [Toyc_compiler__Ast.Expr (Toyc_compiler__Ast.Int 1)]) in
  let main_body = Toyc_compiler__Ast.Block [make_return 0] in
  let prog = [foo; make_main_func main_body] in
  assert_semantic_error ~expect_not_all_paths_return:true "int_func_no_return" prog "int 类型函数 foo 不是所有路径都返回值"

let test_div_by_zero _ =
  let body = Toyc_compiler__Ast.Block [
    make_var_decl "a" 10;
    make_assign "a" 0;
    Toyc_compiler__Ast.Expr (Toyc_compiler__Ast.BinaryOp (Toyc_compiler__Ast.Div, Toyc_compiler__Ast.Int 1, Toyc_compiler__Ast.Int 0));
    make_return 0
  ] in
  let prog = [make_main_func body] in
